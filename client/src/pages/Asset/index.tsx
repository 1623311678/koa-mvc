...
const ModalForm: React.FC<any> = ({ open, onCancel, getData, userId }) => {
  ...
  const onOk = () => {
    form
      ...
      .then(async values => {
        setModalLoading(true)
        const { name, name2, type, contact } = values
        if (userId) {
          const res: any = await post(apiMap.createAsset, {
            name,
            name2,
            type,
            contact
...
function AssetList() {
  ...
  const getData = async (params: any = {}) => {
    ...
    const res: any = await post(apiMap.getAssetList, {
      pageSize,
      pageNumber,
      userName,
      phoneNumber
  ...
  return (
    <div>
      <Breadcrumb>
        <Breadcrumb.Item>资产配置管理</Breadcrumb.Item>
      </Breadcrumb>
      <Form
        name="basic"
        labelCol={{ span: 8 }}
